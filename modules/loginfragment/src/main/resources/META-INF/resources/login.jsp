			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					<%
						...
						String userType = httpOrigReq.getParameter("userType");
						String loginLabel = null;
	
						if (userType != null && userType.equals("NormalUser")) {
							loginLabel = "nid";
						}
						else {
					...
					<liferay-ui:error key="loginError" message="login-error" />
					<aui:input autoFocus="<%= windowState.equals(LiferayWindowState.EXCLUSIVE) || windowState.equals(LiferayWindowState.MAXIMIZED) %>" cssClass="clearable" label="<%= loginLabel %>" name="login" showRequiredLabel="<%= false %>" type="text" value="<%= login %>">

						<c:if test="<%= !"NormalUser".equals(userType) %>">
							<aui:validator name="required" errorMessage="the-email-field-is-required"> </aui:validator>
							<aui:validator name="email" errorMessage="please-enter-a-valid-email-address"></aui:validator>
						</c:if>
						<c:if test="<%= "NormalUser".equals(userType) %>">
							<aui:validator name="required" errorMessage="the-nid-field-is-required"> </aui:validator>
							<aui:validator name="number" errorMessage="please-enter-a-valid-nid"></aui:validator>
						</c:if>
					</aui:input>
				...
				<aui:button-row>
					<aui:button type="submit" value="sign-in" cssClass="m-3" />
				</aui:button-row>
			</aui:form>
