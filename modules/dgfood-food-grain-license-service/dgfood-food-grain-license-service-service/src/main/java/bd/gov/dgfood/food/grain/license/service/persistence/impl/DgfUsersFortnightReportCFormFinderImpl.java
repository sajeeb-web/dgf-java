...
@Component(service = DgfUsersFortnightReportCFormFinder.class)
public class DgfUsersFortnightReportCFormFinderImpl extends DgfUsersFortnightReportCFormFinderBaseImpl implements DgfUsersFortnightReportCFormFinder{
...
public List<Map<String, Object>> findBySearchCriteria(String bussinessType,String endDate, long subdistrict, long union) {
	...
	try {
		...
		DSLQuery dslQuery = DSLQueryFactoryUtil.select(DgfUsersFortnightReportCFormTable.INSTANCE.reportId,DgfUsersFortnightReportCFormTable.INSTANCE.bussinessType,  
				DgfUsersFortnightReportCFormTable.INSTANCE.screenName,DgfUsersFortnightReportCFormTable.INSTANCE.endDate,  dgfLocUnionTable.INSTANCE.name, dgfLocSubdistrictTable.INSTANCE.name.as("upazilaName")).
				from(DgfUsersFortnightReportCFormTable.INSTANCE).innerJoinON(licnsUserRegsTable.INSTANCE, licnsUserRegsTable.INSTANCE.nationalId.eq(DgfUsersFortnightReportCFormTable.INSTANCE.usernationalId))
				...
				.innerJoinON(ResourcePermissionTable.INSTANCE, ResourcePermissionTable.INSTANCE.primKeyId.eq(DgfUsersFortnightReportCFormTable.INSTANCE.reportId))
...
private Predicate gePredicate(String bussinessType, long subdistrict, long union) {
	...
	Predicate predicate = DgfUsersFortnightReportCFormTable.INSTANCE.reportId.neq(0L);
	if(bussinessType != null && !bussinessType.isEmpty()) {
		predicate = predicate.and(DgfUsersFortnightReportCFormTable.INSTANCE.bussinessType.eq(bussinessType));
	}
